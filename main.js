...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("787565056506069012").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("740625895181516831").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    .setFooter('Будь всегда на позитиве :3', 'https://cdn.discordapp.com/emojis/590614597610766336.gif?v=1')
    // .addField(`Участвуй в розыгрышах!`, `<740628277894316103>`, true) // Добавляйте свои каналы по желанию
    // .addField(`Общайся в чате!`, `<740630530768371712>`, true)
    // .addField(`Смотри видео наших ютуберов!`, `<740628623727263904>`, true)
    .setColor('RANDOM')
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('787565056506069012').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("740625895181516831").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("787565614038253589").setName(`На сервере: ${members}`); // свой айди войса
}
